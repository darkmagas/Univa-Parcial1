...
public class EnemyManager : MonoBehaviour
{
    ...
    private IEnumerator CreateWave()
    {
        ...
        yield return StartCoroutine(SpawnEnemies(wave.midEnemy, _weakEnemyPrefab));
        yield return StartCoroutine(SpawnEnemies(wave.strongEnemy, _weakEnemyPrefab));
        _currentWave++;
    ...
    private IEnumerator SpawnEnemies(int amount, GameObject prefab)
    {
        for (int i = 0; i < amount; i++)
        {
            ...
            EventDispatcher.Dispatch(signal: new SpawnObject(prefab,null,_spawnpoints[randomSpawn].position,Quaternion.identity, (gameObjectSpawn)=> { int rs = randomSpawn; string pathName = _pathNames[rs]; gameObjectSpawn.GetComponent<FollowPathMovement>().InitEnemy(pathName); }));
